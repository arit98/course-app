...
const SignInModal = () => {
  ...
  const { isSignInOpen, setIsLoggedIn, setSignInOpen } = useContext(AuthContext);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if(err.code==="auth/invalid-credential"){
        ...
        setError("Too many attempts. Try again after sometime")
      }
    }
  };
    ...
    <div className="modal-overlay" onClick={()=>setSignInOpen(false)}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        >
          ...
          <p className="md:hidden flex text-center text-red-500" onClick={() => setSignInOpen(false)}>close the modal</p>
        </form>
      </div>
    </div>
  );
